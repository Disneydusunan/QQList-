...
-(NSArray *)dataArray{
    if (!_dataArray) {
        ...
        for(NSDictionary *dic in arr){
            JKGroupModel *group = [JKGroupModel GroupWithDict:dic];
...
{
    JKGroupModel *g = _dataArray[section];
    NSInteger count = g.isOpen?g.friends.count:0;
    return count;
}
//
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JKGroupModel *g = _dataArray[indexPath.section];
    JKFriendsModel *m = g.friends[indexPath.row];
    
    cell.imageView.image = [UIImage imageNamed:m.icon];
    cell.textLabel.text = m.name;
    cell.detailTextLabel.text = m.intro;
